...
public class Movement
{
    ...
    public Movement Wait(float duration = 1)
    {
        var step = new MovementStep(duration, true, false, t => {});
        steps[steps.Count - 1].onEnd = delegate { step.runStep(mb); };
        steps.Add(step);
        return this;
    }
    public Movement PlayAndWait(TranslatableAudioClip clip, AudioTranslator audioTranslator, float minWaitTime = 0)
    {
        var step = new MovementStep(() => audioTranslator.Play(clip));
        steps[steps.Count - 1].onEnd = delegate { step.runStep(mb); };
        steps.Add(step);
        var waitTime = Mathf.Max(minWaitTime, audioTranslator.getLength(clip));
    ...
    public class MovementStep
    {
        ...
        public MovementStep(float duration = 1, bool linear = false, bool reverse = false, params Action<float>[] animFunc)
        {
            ...
            this.curve = linear ? AnimationCurve.Linear(0, 0, duration, 1) : AnimationCurve.EaseInOut(0, 0, duration, 1);
