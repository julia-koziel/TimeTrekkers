...
public class AspectUtilityCamera : MonoBehaviour {
	...
	void Awake () {
        ...
        if (!backgroundCam) {
            var go = GameObject.FindGameObjectWithTag("BackgroundCamera");
            if (go) backgroundCam = go.GetComponent<Camera>();
        }
	...
	void SetCamera () {
		...
		if (!backgroundCam) {
            ...
            backgroundCam.tag = "BackgroundCamera";
            var go = GameObject.Find("Cameras");
            if (go) backgroundCam.transform.SetParent(go.transform);
            backgroundCam.transform.SetSiblingIndex(0);
		}
