...
public class RLSP_Stimuli_Manager_new : MonoBehaviour
{
    ...
    public void OnStartTrial()
    {
        ...
        if (participantsGo && trial== 0)
        {
            ...
            unrewardedStimuli[0].transform.position = rightPos.position;
            unrewardedStimuli.ForEach(s => s.SetActive(true));
        }
        else if (participantsGo && trial == 1)
        {
            rewardedStimuli[0].transform.position = rightPos.position;
            rewardedStimuli.ForEach(s => s.SetActive(true));
        ...
        else if (trial <40)
        {
            if (probindex==0)
            {
                ...
                var rewarded = rewardedStimuli[Random.Range(0, 1)];
                var unrewarded = unrewardedStimuli[Random.Range(0, 1)];
                ...
                if (probindex==0)
            {
                ...
                var rewarded = unrewardedStimuli[Random.Range(0, 1)];
                var unrewarded = rewardedStimuli[Random.Range(0, 1)];
    ...
    public void OnEndResponseWindow()
    {
        if (participantsGo)
        {
            ...
            if (trial == 0)
            {
                ...
                rewardedStimulus = rewardedStimuli.First(s => s.id == clickedId.Value);
                side.Value = rewardedStimuli.IndexOf(swapStimulus) == 0 ? RIGHT : LEFT;

                rewardedStimuli.Remove(swapStimulus);
                unrewardedStimuli.Remove(rewardedStimulus);
